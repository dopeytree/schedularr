...
const App = () => {
  ...
  useEffect(() => {
    fetch('/config.json')
      ...
      .then(config => {
        ...
        setLogoSrc(`${normalizedBaseUrl}assets/logo.svg`);
      })
      .catch(error => {
        console.error('Error loading config:', error);
        // Fallback to window.location.origin
        const fallbackBaseUrl = window.location.origin.endsWith('/') ? window.location.origin : `${window.location.origin}/`;
        setBaseUrl(fallbackBaseUrl);
        setLogoSrc(`${fallbackBaseUrl}assets/logo.svg`);
      });
  }, []);
  ...
  const exchangeCodeForTokens = async (code) => {
    try {
      const redirectUri = baseUrl || window.location.origin;
      console.log('Using redirectUri:', redirectUri); // Debug
  ...
  const handleSignIn = async () => {
    ...
    authUrl.searchParams.append('redirect_uri', baseUrl || window.location.origin);
    ...
    console.log('Redirecting to auth URL:', authUrl.toString()); // Debug
          ...
          <div className="relative mx-auto mb-4 w-16 h-16">
            {logoSrc ? (
              <img
                ...
                style={{ filter: 'invert(0) brightness(8) sepia(10) hue-rotate(166deg) saturate(10)' }}
                onError={() => console.error('Failed to load logo at:', logoSrc)}
              />
            ) : (
              <p className="text-red-400">Loading logo...</p>
            )}
          </div>
