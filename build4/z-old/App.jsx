...
const App = () => {
  ...
  const [baseUrl, setBaseUrl] = useState(''); // State for dynamic base URL
  const [logoSrc, setLogoSrc] = useState(''); // State for logo URL

  // PKCE state
  const [codeVerifier, setCodeVerifier] = useState('');
  const [state, setState] = useState('');

  // Fetch config.json to set baseUrl
  useEffect(() => {
    fetch('/config.json')
      .then(response => {
        if (!response.ok) {
      ...
      .then(config => {
        ...
        setLogoSrc(`${config.baseUrl}/assets/logo.svg`); // Set logo URL dynamically
      })
      .catch(error => {
        console.error('Error loading config:', error);
        // Fallback to window.location.origin if config fails
        setBaseUrl(window.location.origin);
        setLogoSrc(`${window.location.origin}/assets/logo.svg`);
      });
  }, []);
  ...
  useEffect(() => {
    ...
    if (code && stateParam && stateParam === state) {
  ...
  const exchangeCodeForTokens = async (code) => {
    try {
      const redirectUri = baseUrl || window.location.origin; // Use baseUrl from config
  ...
  const handleSignIn = async () => {
    if (!validateClientId(clientId)) {
      setSubmissionError('Invalid Client ID. Please check Settings.');
      return;
    }
    ...
    authUrl.searchParams.append('redirect_uri', baseUrl || window.location.origin); // Use baseUrl
  ...
  useEffect(() => {
    ...
    if (codeVerifier) localStorage.setItem('codeVerifier', codeVerifier);
    if (state) localStorage.setItem('state', state);
  }, [hourlyRate, whoWhat, calendarId, clientId, devMode, codeVerifier, state]);

  // Auto-calculate end time, date, and cost
  useEffect(() => {
    if (event.startDate && event.startTime && event.duration) {
      ...
      if (isNaN(start.getTime())) {
        console.error('Invalid date format:', `${event.startDate}T${event.startTime}`);
        setTimeError('Invalid date or time format. Please use YYYY-MM-DD for date and HH:MM for time.');
        return;
      }
      ...
      {!isSignedIn ? (
        <div className="text-center">
          ...
          <div className="relative mx-auto mb-4 w-16 h-16">
            <img src={logoSrc} alt="Schedularr Logo" className="w-full h-full" style={{ filter: 'invert(0) brightness(8) sepia(10) hue-rotate(166deg) saturate(10)' }} />
          </div>
        ...
        <>
                  ...
                  <div className="space-y-6">
                    ...
                    <div className="text-sm text-gray-300 bg-gray-700/50 p-4 rounded-lg">
                      ...
                      <ol className="list-decimal pl-5 space-y-2">
                        ...
                        <li>Create OAuth 2.0 Client ID: Select “Web application”, add JavaScript origins (e.g., https://schedularr.me, http://localhost:5173) and redirect URIs (e.g., https://schedularr.me, http://localhost:5173), and copy the Client ID.</li>
                        ...
                        <li>If the event doesn’t appear, check:
                          <ul className="list-disc pl-5 mt-1">
                            ...
                            <li>JavaScript origins and redirect URIs match your app’s URL.</li>
          ...
          {showCalendar && (
            <div className="mt-8 w-full max-w-4xl">
              <iframe
                key={calendarKey}
                src={calendarId ? `${baseUrl}/calendar/embed?src=${encodeURIComponent(calendarId)}&ctz=UTC` : `${baseUrl}/calendar/embed?src=&ctz=UTC`}
                style={{ border: 0 }}
                width="100%"
                height="400"
                frameborder="0"
