...
const App = () => {
  ...
  // Rocket animation (looping until login, centered on screen)
  useEffect(() => {
    let intervalId;
    if (!isSignedIn && isAnimating) {
      const animate = () => {
        const blastYPos = window.innerHeight / 2; // Center vertically
        const blastXPos = window.innerWidth / 2; // Center horizontally
        const rocket = () => {
          emojiBlast({
            emojiCount: 1,
            emojis: ["🚀"],
