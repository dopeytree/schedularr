...
const CardTitle = ({ className, inputGradient, ...props }) => {
  const gradientStyles = {
    'white-yellow': 'bg-gradient-to-r from-yellow-300 to-yellow-500 bg-clip-text text-transparent',
...
const Alert = ({ className, variant = "default", inputGradient, ...props }) => {
  const baseStyles = "relative w-full rounded-lg border p-4 text-center";
  const variants = {
  ...
  return (
    <div
      ...
      className={cn(
        baseStyles,
        variant === "success" ? (inputGradient === "white-yellow" ? "bg-gradient-to-r from-yellow-500 to-yellow-600" : inputGradient === "green-cyan" ? "bg-gradient-to-r from-green-500 to-cyan-600" : "bg-gradient-to-r from-blue-600 to-purple-600") : variants[variant],
        className
      )}
      {...props}
    />
...
const TabsTrigger = ({ value, onValueChange, tabValue, className, variant = "gradient", inputGradient, ...props }) => {
  ...
  const variants = {
    gradient: inputGradient === "white-yellow" ? "bg-gradient-to-r from-yellow-500 to-yellow-600 text-white hover:from-yellow-600 hover:to-yellow-700 focus:ring-yellow-500 transform hover:scale-105" :
    ...
    gradientReverse: inputGradient === "white-yellow" ? "bg-gradient-to-l from-yellow-500 to-yellow-600 text-white hover:from-yellow-600 hover:to-yellow-700 focus:ring-yellow-500 transform hover:scale-105" :
  ...
  return (
    <button
      className={cn(
        baseStyles,
        variants[variant],
        value === tabValue ? (inputGradient === "white-yellow" ? "bg-yellow-800 text-yellow-300" : inputGradient === "green-cyan" ? "bg-green-800 text-cyan-300" : "bg-indigo-800 text-cyan-300") : "",
        className
      )}
...
const App = () => {
  ...
  const [sessionExpiryDuration, setSessionExpiryDuration] = useState(() => {
    return localStorage.getItem('sessionExpiryDuration') || '2592000000'; // 30 days
  });
  ...
  }, [isSignedIn, userEmail, userName, accessToken, hourlyRate, currency, whoWhat, calendarId, inputGradient, sessionExpiryDuration, devMode]);

  // Rocket animation (looping until login, flies from bottom left to top right)
  useEffect(() => {
    let intervalId;
    if (!isSignedIn && isAnimating) {
      const animate = () => {
        const blastYPos = window.innerHeight - 10; // Bottom of the screen (original position)
        const startXPos = 0; // Left of the screen
        const rocket = () => {
          emojiBlast({
            emojiCount: 1,
            emojis: ["🚀"],
            physics: {
              ...
              gravity: 0,
              initialVelocities: {
                x: 15, // Move right (slightly slower than before)
                y: -20 // Move up (stronger upward movement)
              },
              friction: 1, // No slowdown
              ticks: 600, // Extended duration for full diagonal travel
              rotation: 45, // Angle the rocket for diagonal movement
      ...
      {!isSignedIn ? (
        ...
        <>
          ...
          <Tabs value={activeTab} onValueChange={setActiveTab}>
            <TabsList className="mb-6 w-full max-w-[400px] mx-auto">
              <TabsTrigger tabValue="booking" value={activeTab} onValueChange={setActiveTab} variant="gradient" inputGradient={inputGradient}>
                <span className={cn(
                  "material-icons mr-2 transition-transform duration-300 hover:scale-125 p-1",
                  inputGradient === "white-yellow" ? "text-yellow-400" : inputGradient === "green-cyan" ? "text-green-400" : "text-blue-400"
                )}>event</span> Booking
              </TabsTrigger>
            ...
            <TabsContent tabValue="booking" value={activeTab}>
              <Card>
                ...
                <CardContent>
                  ...
                  {showConfetti && (
                    <Alert variant="success" inputGradient={inputGradient} className="mb-4">Booking Sent! 🎉</Alert>
                  )}
                  <div className="space-y-6">
                    ...
                    <div className="flex items-center">
                      ...
                      <div className="relative w-full">
                        ...
                        <Input
                          ...
                          readOnly
                          placeholder="Cost"
                          className={cost ? 'text-white pl-8' : 'text-gray-400 pl-8'}
                          inputGradient={inputGradient}
                        />
                      </div>
                    </div>
                    <div className="flex items-center">
                      ...
                      <Input
                        ...
                        value={event.note}
                        onChange={handleInputChange}
                        placeholder="Short note (e.g., 'Bring snacks')"
                        maxLength="20"
                        inputGradient={inputGradient}
                      />
                    </div>
            ...
            <TabsContent tabValue="settings" value={activeTab}>
              <Card>
                <CardHeader>
                  <CardTitle inputGradient={inputGradient}>Admin</CardTitle>
                </CardHeader>
                <CardContent>
                  <div className="space-y-6">
                    <div className="flex items-center">
                    ...
                    <div className="flex items-center">
                      ...
                      {calendarId ? (
                        <p className="text-white cursor-pointer hover:underline" onClick={openCalendarLink}>Using calendar: Schedularr</p>
                      ) : (
                    ...
                    {shareSuccess && (
                      <Alert variant="success" inputGradient={inputGradient} className="mt-2">{shareSuccess}</Alert>
                    )}
                    <div className="flex items-center">
                      <span className={cn(
                        "material-icons mr-3 transition-transform duration-300 hover:scale-125 p-1",
          ...
          <Button
            ...
            variant="gradient"
            className="w-full py-3 text-base font-semibold flex items-center justify-center mt-6"
          >
            <span className={cn(
              "material-icons mr-2 transition-transform duration-300 hover:scale-125 p-1",
              inputGradient === "white-yellow" ? "text-yellow-400" : inputGradient === "green-cyan" ? "text-green-400" : "text-blue-400"
